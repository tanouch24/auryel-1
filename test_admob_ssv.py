...
class AdmobSsvVerifierTest(unittest.TestCase):
    ...
    def test_valid_signature_preserves_raw_query(self):
        raw = self._callback(
            b"ad_network=5450213213286189855&ad_unit=unit&reward_amount=12"
    ...
    def test_tampering_is_rejected(self):
        raw = self._callback(b"ad_unit=unit&reward_amount=12&transaction_id=tx")
        with self.assertRaises(admob_ssv.SsvError):
            admob_ssv.verify_callback(
                raw.replace(b"reward_amount=12", b"reward_amount=60"),
    ...
    def test_duplicate_query_parameter_rejected(self):
        content = b"ad_unit=unit&ad_unit=other&reward_amount=12&transaction_id=tx"
    ...
    def test_reward_v5_migration_is_12_and_has_no_daily_cap(self):
        with open("migrations/032_rewarded_ad_12_stars.sql", encoding="utf-8") as handle:
            migration = handle.read()
        self.assertIn("stars_amount = 12", migration)
        self.assertIn("daily_limit = NULL", migration)
